      ...
      >
        <div class="flex flex-col space-y-2 pt-4">
          <a
            ...
            :key="item.id"
            :href="`#${item.id}`"
            class="link cursor-glow text-base font-medium focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0a0a] rounded block py-2"
            @click="closeMobileMenu"
          >
            {{ item.label }}
          </a>
          ...
          <div class="flex items-center gap-2 pt-3">
            <button
              ...
              :key="loc"
              type="button"
              class="px-3 py-2 rounded-full text-xs font-semibold border transition-colors flex-1 text-center focus:outline-none focus-visible:ring-2 focus-visible:ring-accent-400"
              :class="
                locale === loc
                  ? 'bg-accent-600 text-white border-accent-600'
                  : 'bg-transparent text-text-secondary border-border-primary hover:border-accent-400'
