...
describe("consent storage", () => {
  ...
  it("creates a decision with version and timestamp (accept all)", () => {
    const state = createConsentState({ statistics: true });
  ...
  it("creates a reject-all decision with statistics disabled", () => {
    const state = createConsentState({ statistics: false });
  ...
  it("supports individual selection (statistics only when chosen)", () => {
    const off = createConsentState({ statistics: false });
    const on = createConsentState({ statistics: true });
  ...
  it("rejects malformed cookie payloads", () => {
    ...
    expect(
      parseConsentCookieValue(
        JSON.stringify({ version: CONSENT_VERSION, decidedAt: "x", categories: { necessary: true } }),
      ),
    ).toBeNull();
  });
});
...
describe("consent decision scenarios", () => {
  ...
  it("Alle ablehnen: statistics stays false", () => {
    const rejected = createConsentState({ statistics: false });
  ...
  it("Alle akzeptieren: statistics enabled", () => {
    const accepted = createConsentState({ statistics: true });
    expect(accepted.categories.statistics).toBe(true);
  });

  it("Widerruf: saving statistics false disables optional category", () => {
    const revoked = createConsentState({ statistics: false });
  ...
  it("Neue Consent-Version: banner must reappear (invalid stored state)", () => {
    const stored = {
      ...
      categories: { necessary: true as const, statistics: true },
    };
